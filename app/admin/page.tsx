  ...
  return (
    <div style={{ minHeight: '100vh', background: '#C8D5D2', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 24px', display: 'grid', gridTemplateColumns: selectedProfile ? '1fr 380px' : '1fr', gap: '24px', alignItems: 'start' }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-aboreto), sans-serif', fontSize: '24px', color: '#0A0A0A', letterSpacing: '1.5px', marginBottom: '20px' }}>ADMIN DASHBOARD</h1>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '24px' }}>
            {[
              ...
              <div key={s.label} style={{ background: '#E8F0EE', border: '1px solid rgba(28,43,50,0.1)', borderRadius: '10px', padding: '16px' }}>
                ...
                <p style={{ fontFamily: 'var(--font-aboreto), sans-serif', fontSize: '28px', color: '#0A0A0A', margin: 0 }}>{s.value}</p>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(28,43,50,0.1)', marginBottom: '20px' }}>
            {(['tradies', 'clients', 'jobs', 'metrics'] as const).map(t => (
              <button key={t} type="button" onClick={() => { setTab(t); setPage(0) }}
                style={{ padding: '10px 20px', border: 'none', borderBottom: tab === t ? '2px solid #D4522A' : '2px solid transparent', background: 'transparent', cursor: 'pointer', fontSize: '13px', fontWeight: tab === t ? 600 : 400, color: tab === t ? '#0A0A0A' : '#7A9098', textTransform: 'capitalize' as const }}>
              ...
              {tradies.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map(t => (
                <div key={t.id} onClick={() => selectProfile(t, 'tradie')}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' as const }}>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontFamily: 'var(--font-aboreto), sans-serif', fontSize: '15px', color: '#0A0A0A', marginBottom: '3px' }}>{t.business_name}</p>
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' as const, flexShrink: 0 }}>
                      <button type="button" onClick={e => { e.stopPropagation(); verifyTradie(t.id, 'licence_verified', !t.licence_verified) }} style={{ fontSize: '11px', padding: '5px 10px', borderRadius: '6px', border: '1px solid rgba(28,43,50,0.2)', background: '#C8D5D2', cursor: 'pointer', color: '#0A0A0A' }}>{t.licence_verified ? 'Unverify licence' : 'Verify licence'}</button>
                      <button type="button" onClick={e => { e.stopPropagation(); verifyTradie(t.id, 'insurance_verified', !t.insurance_verified) }} style={{ fontSize: '11px', padding: '5px 10px', borderRadius: '6px', border: '1px solid rgba(28,43,50,0.2)', background: '#C8D5D2', cursor: 'pointer', color: '#0A0A0A' }}>{t.insurance_verified ? 'Unverify insurance' : 'Verify insurance'}</button>
              ...
              {tradies.length > PAGE_SIZE && (
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:'16px', padding:'10px 0' }}>
                  <button type="button" onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                    style={{ fontSize:'13px', color:'#0A0A0A', background:'#E8F0EE', border:'1px solid rgba(28,43,50,0.15)', borderRadius:'6px', padding:'7px 14px', cursor: page === 0 ? 'not-allowed' : 'pointer', opacity: page === 0 ? 0.4 : 1 }}>Prev</button>
                  ...
                  <button type="button" onClick={() => setPage(p => p + 1)} disabled={(page + 1) * PAGE_SIZE >= tradies.length}
                    style={{ fontSize:'13px', color:'#0A0A0A', background:'#E8F0EE', border:'1px solid rgba(28,43,50,0.15)', borderRadius:'6px', padding:'7px 14px', cursor: (page + 1) * PAGE_SIZE >= tradies.length ? 'not-allowed' : 'pointer', opacity: (page + 1) * PAGE_SIZE >= tradies.length ? 0.4 : 1 }}>Next</button>
                </div>
              )}
            </div>
          )}
              ...
              {clients.map(c => (
                <div key={c.id} onClick={() => selectProfile(c, 'client')}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                    <div>
                      <p style={{ fontSize: '14px', fontWeight: 500, color: '#0A0A0A', marginBottom: '3px' }}>{c.full_name}</p>
          ...
          {tab === 'jobs' && (
            <div>
              <input type="text" placeholder="Filter by title, suburb, client, tradie..." value={jobFilter} onChange={e => setJobFilter(e.target.value)}
                style={{ width:'100%', padding:'10px 14px', borderRadius:'8px', border:'1px solid rgba(28,43,50,0.15)', fontSize:'13px', background:'white', marginBottom:'14px', boxSizing:'border-box' as const, color:'#0A0A0A' }} />
                  ...
                  <div key={j.id} style={{ background: j.status === 'disputed' ? 'rgba(212,82,42,0.04)' : '#E8F0EE', border: selectedJob?.id === j.id ? '1.5px solid #D4522A' : j.status === 'disputed' ? '1px solid rgba(212,82,42,0.3)' : '1px solid rgba(28,43,50,0.1)', borderRadius: '10px', padding: '14px 18px' }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'12px', flexWrap:'wrap' as const }}>
                      <div style={{ flex:1, cursor:'pointer' }} onClick={() => loadJobThread(j)}>
                        <p style={{ fontSize: '13px', fontWeight: 500, color: '#0A0A0A', marginBottom: '3px' }}>{j.title}</p>
                      ...
                      <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexShrink: 0, flexWrap:'wrap' as const }}>
                        ...
                        <button type="button" onClick={() => loadJobThread(j)} style={{ fontSize:'11px', padding:'4px 10px', borderRadius:'6px', border:'1px solid rgba(28,43,50,0.2)', background:'white', cursor:'pointer', color:'#0A0A0A' }}>View thread</button>
                      ...
                      <div style={{ marginTop:'14px', borderTop:'1px solid rgba(28,43,50,0.08)', paddingTop:'14px' }}>
                        ...
                        <div style={{ maxHeight:'320px', overflowY:'auto' as const, display:'flex', flexDirection:'column' as const, gap:'8px', marginBottom:'12px' }}>
                          ...
                          {jobMessages.map((msg: any) => {
                            ...
                            return (
                              <div key={msg.id} style={{ padding:'8px 12px', borderRadius:'8px', background: isAdmin ? '#0A0A0A' : 'white', border:'1px solid rgba(28,43,50,0.08)', maxWidth:'80%', alignSelf: isAdmin ? 'flex-end' as const : 'flex-start' as const }}>
                                ...
                                <p style={{ fontSize:'12px', color: isAdmin ? 'rgba(216,228,225,0.9)' : '#0A0A0A', margin:0, lineHeight:'1.5' }}>{msg.body}</p>
                              </div>
                            )
                          })}
                        </div>
                        <div style={{ display:'flex', gap:'8px' }}>
                          <input type="text" value={adminMessage} onChange={e => setAdminMessage(e.target.value)}
                            ...
                            style={{ flex:1, padding:'9px 12px', borderRadius:'7px', border:'1px solid rgba(28,43,50,0.2)', fontSize:'12px', background:'white', color:'#0A0A0A' }} />
                          <button type="button" onClick={postAsAdmin} disabled={sendingMsg || !adminMessage.trim()}
                            style={{ padding:'9px 16px', background:'#0A0A0A', color:'white', border:'none', borderRadius:'7px', fontSize:'12px', fontWeight:500, cursor:'pointer', opacity: sendingMsg || !adminMessage.trim() ? 0.5 : 1 }}>
                            {sendingMsg ? '...' : 'Send'}
                          </button>
                        </div>
                      </div>
            ...
            <div>
              ...
              <div style={{ background:'#E8F0EE', border:'1px solid rgba(28,43,50,0.1)', borderRadius:'12px', overflow:'hidden' }}>
                <div style={{ padding:'14px 18px', borderBottom:'1px solid rgba(28,43,50,0.08)' }}>
                  <p style={{ fontFamily:'var(--font-aboreto), sans-serif', fontSize:'13px', color:'#0A0A0A', letterSpacing:'0.5px', margin:0 }}>JOBS BY STATUS</p>
                </div>
                <div style={{ padding:'14px 18px', display:'flex', flexDirection:'column' as const, gap:'8px' }}>
                  {Object.entries(metrics.statusCounts).sort(([,a],[,b]) => (b as number) - (a as number)).map(([status, count]) => (
                    <div key={status} style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                      ...
                      <span style={{ fontSize:'12px', color:'#0A0A0A', fontWeight:500, width:'30px', textAlign:'right' as const }}>{count as number}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {selectedProfile && (
          <div style={{ position: 'sticky' as const, top: '80px', display: 'flex', flexDirection: 'column' as const, gap: '14px' }}>
                ...
                {selectedProfile.type === 'tradie' ? (
                  [
                    ...
                    <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid rgba(28,43,50,0.06)', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '12px', fontWeight: 500, color: '#0A0A0A', textAlign: 'right' as const }}>{String(item.value)}</span>
                    </div>
                  ))
                ) : (
                  [
                    ...
                    <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid rgba(28,43,50,0.06)', gap: '8px' }}>
                      <span style={{ fontSize: '11px', color: '#7A9098', flexShrink: 0 }}>{item.label}</span>
                      <span style={{ fontSize: '12px', fontWeight: 500, color: '#0A0A0A' }}>{String(item.value)}</span>
                    </div>
                  ))
                )}
              </div>
            ...
            {profileJobs.length > 0 && (
              <div style={{ background: '#E8F0EE', border: '1px solid rgba(28,43,50,0.1)', borderRadius: '12px', overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(28,43,50,0.08)' }}>
                  <p style={{ fontFamily: 'var(--font-aboreto), sans-serif', fontSize: '12px', color: '#0A0A0A', letterSpacing: '0.5px', margin: 0 }}>JOB HISTORY ({profileJobs.length})</p>
                </div>
                <div style={{ padding: '10px 16px', maxHeight: '200px', overflowY: 'auto' as const }}>
                  {profileJobs.map(j => (
                    <div key={j.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid rgba(28,43,50,0.06)', gap: '8px' }}>
                      <div>
                        <p style={{ fontSize: '12px', fontWeight: 500, color: '#0A0A0A', margin: 0 }}>{j.title}</p>
            ...
            <div style={{ background: '#E8F0EE', border: '1px solid rgba(28,43,50,0.1)', borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(28,43,50,0.08)' }}>
                <p style={{ fontFamily: 'var(--font-aboreto), sans-serif', fontSize: '12px', color: '#0A0A0A', letterSpacing: '0.5px', margin: 0 }}>ADMIN NOTES</p>
              </div>
              <div style={{ padding: '12px 16px' }}>
                <div style={{ marginBottom: '12px', maxHeight: '200px', overflowY: 'auto' as const }}>
                  {notes.length === 0 ? (
                    ...
                    <div key={n.id} style={{ padding: '8px 10px', background: '#C8D5D2', borderRadius: '8px', marginBottom: '8px' }}>
                      <p style={{ fontSize: '12px', color: '#0A0A0A', margin: '0 0 4px', lineHeight: '1.5' }}>{n.note}</p>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button type="button" onClick={saveNote} disabled={savingNote || !newNote.trim()}
                    style={{ background: '#0A0A0A', color: 'white', padding: '8px 12px', borderRadius: '7px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 500, opacity: savingNote || !newNote.trim() ? 0.5 : 1, alignSelf: 'flex-end' as const, flexShrink: 0 }}>
                    {savingNote ? '...' : 'Save'}
                  </button>
                </div>
              </div>
