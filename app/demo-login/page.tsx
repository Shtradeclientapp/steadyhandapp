...
export default function DemoLoginPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', fontFamily: 'sans-serif', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
      <p style={{ fontFamily: 'var(--font-aboreto), sans-serif', fontSize: '11px', color: 'rgba(216,228,225,0.3)', letterSpacing: '2px', marginBottom: '20px' }}>STEADYHAND DEMO</p>
      <h1 style={{ fontFamily: 'var(--font-aboreto), sans-serif', fontSize: '28px', color: 'rgba(216,228,225,0.9)', letterSpacing: '1px', marginBottom: '12px', textAlign: 'center' as const }}>Choose your perspective</h1>
      <p style={{ fontSize: '14px', color: 'rgba(216,228,225,0.4)', marginBottom: '40px', textAlign: 'center' as const, maxWidth: '480px', lineHeight: '1.7' }}>
          ...
          <button key={account.role} type="button" onClick={() => login(account)} disabled={!!loading}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                <span style={{ fontSize: '14px', fontWeight: 600, color: account.color }}>{account.label}</span>
                <span style={{ fontSize: '11px', color: 'rgba(216,228,225,0.35)' }}>— {account.name}</span>
              </div>
              <p style={{ fontSize: '12px', color: 'rgba(216,228,225,0.5)', lineHeight: '1.55', margin: 0 }}>{account.desc}</p>
            </div>
            <span style={{ fontSize: '16px', color: 'rgba(216,228,225,0.2)', flexShrink: 0, marginTop: '2px' }}>→</span>
          </button>
        ))}
      </div>
      ...
      <p style={{ marginTop: '32px', fontSize: '11px', color: 'rgba(216,228,225,0.2)', textAlign: 'center' as const, lineHeight: '1.6' }}>
        This is a demo environment. No real data, no real payments.<br />
        <a href="https://steadyhandtrade.app/signup" style={{ color: 'rgba(216,228,225,0.3)', textDecoration: 'none' }}>Create a real account →</a>
      </p>
    </div>
  )
}
