...
export default function WorkersPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const { data: prof } = await supabase.from('profiles').select('*, tradie:tradie_profiles(business_name, id, worker_seats_included, worker_seats_extra, subscription_tier)').eq('id', session.user.id).single()
