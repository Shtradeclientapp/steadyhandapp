...
export default function WorkersPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const { data: prof } = await supabase.from('profiles').select('id, email, full_name, role, tradie:tradie_profiles!tradie_profiles_id_fkey(business_name, id, worker_seats_included, worker_seats_extra, subscription_tier)').eq('id', session.user.id).single()
