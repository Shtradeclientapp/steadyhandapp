...
export default function OrgRequestPage() {
  ...
  const [form, setForm] = useState({ title:'', description:'', trade_category:'Electrical', urgency:'Within 2 weeks', property_type:'Residential', warranty_period:'90' })
  ...
  const submitJobs = async () => {
    ...
    for (const propId of selectedProperties) {
      ...
      const { error } = await supabase.from('jobs').insert({
        ...
        property_type: prop?.property_type || form.property_type,
        suburb: prop?.suburb || '',
  ...
  if (submitted) return (
    <div style={{ minHeight:'100vh', background:'#C8D5D2', fontFamily:'sans-serif', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ background:'white', borderRadius:'16px', padding:'48px', textAlign:'center' as const, maxWidth:'480px', width:'100%', margin:'0 24px' }}>
        <div style={{ fontSize:'48px', marginBottom:'16px' }}>✓</div>
        <h2 style={{ fontFamily:'var(--font-aboreto), sans-serif', fontSize:'20px', color:'#1C2B32', marginBottom:'8px' }}>Jobs created</h2>
        <p style={{ fontSize:'14px', color:'#4A5E64', marginBottom:'24px' }}>{selectedProperties.length} job request{selectedProperties.length !== 1 ? 's' : ''} created across {selectedProperties.length} {selectedProperties.length !== 1 ? 'properties' : 'property'}.</p>
  ...
  const bySuburb = properties.reduce((acc: Record<string, any[]>, p) => {
    const key = p.suburb || 'Other'
    if (!acc[key]) acc[key] = []
    acc[key].push(p)
    return acc
    ...
    <div style={{ minHeight:'100vh', background:'#C8D5D2', fontFamily:'sans-serif' }}>
      ...
      <div style={{ maxWidth:'860px', margin:'0 auto', padding:'32px 24px' }}>
        ...
        <div style={{ display:'flex', gap:'8px', marginBottom:'28px' }}>
          {[{id:'select',label:'Select properties'},{id:'details',label:'Job details'},{id:'confirm',label:'Confirm & submit'}].map((s,i) => (
            <div key={s.id} style={{ display:'flex', alignItems:'center', gap:'8px' }}>
              {i > 0 && <div style={{ width:'24px', height:'1px', background:'rgba(28,43,50,0.2)' }} />}
              <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                <div style={{ width:'22px', height:'22px', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'11px', fontWeight:600, background: step === s.id ? '#1C2B32' : 'rgba(28,43,50,0.15)', color: step === s.id ? 'white' : '#7A9098' }}>{i+1}</div>
                <span style={{ fontSize:'12px', color: step === s.id ? '#1C2B32' : '#7A9098', fontWeight: step === s.id ? 600 : 400 }}>{s.label}</span>
              </div>
            </div>
          ))}
        </div>
              ...
              <div style={{ padding:'16px 20px', borderBottom:'1px solid rgba(28,43,50,0.08)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <div style={{ display:'flex', gap:'8px' }}>
                  <button type="button" onClick={() => setSelectedProperties(properties.map(p => p.id))}
                    style={{ fontSize:'12px', color:'#2E6A8F', background:'none', border:'none', cursor:'pointer', padding:'4px 8px' }}>Select all</button>
                  <button type="button" onClick={() => setSelectedProperties([])}
                    style={{ fontSize:'12px', color:'#7A9098', background:'none', border:'none', cursor:'pointer', padding:'4px 8px' }}>Clear</button>
                </div>
              </div>
              {properties.length === 0 ? (
                ...
                <div style={{ padding:'16px 20px' }}>
                  {Object.entries(bySuburb).sort(([a],[b]) => a.localeCompare(b)).map(([suburb, props]) => (
                    <div key={suburb} style={{ marginBottom:'16px' }}>
                      <p style={{ fontSize:'11px', fontWeight:600, color:'#7A9098', letterSpacing:'0.5px', textTransform:'uppercase' as const, marginBottom:'8px' }}>{suburb}</p>
        ...
        {step === 'details' && (
          <div>
            ...
            <div style={{ display:'flex', justifyContent:'space-between' }}>
              <button type="button" onClick={() => setStep('select')} style={{ background:'transparent', color:'#4A5E64', padding:'12px 20px', borderRadius:'8px', fontSize:'14px', fontWeight:400, border:'1px solid rgba(28,43,50,0.2)', cursor:'pointer' }}>← Back</button>
          ...
          <div>
            <div style={{ background:'rgba(192,120,48,0.06)', border:'1px solid rgba(192,120,48,0.25)', borderRadius:'12px', padding:'16px 20px', marginBottom:'20px' }}>
              ...
              <p style={{ fontSize:'13px', color:'#4A5E64', lineHeight:'1.6' }}>
                You are about to post <strong>{selectedProperties.length} separate job request{selectedProperties.length !== 1 ? 's' : ''}</strong> — one for each selected property.
                Each job will be independently managed through the full Steadyhand flow.
                Please ensure the tradie{selectedProperties.length !== 1 ? 's' : ''} you invite have capacity to service each property before committing.
              </p>
            </div>
            ...
            <div style={{ background:'#E8F0EE', border:'1px solid rgba(28,43,50,0.1)', borderRadius:'14px', padding:'20px', marginBottom:'20px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginBottom:'16px' }}>
                {[
                  ...
                  <div key={item.label}>
                    <p style={{ fontSize:'10px', color:'#7A9098', letterSpacing:'0.5px', marginBottom:'2px', textTransform:'uppercase' as const }}>{item.label}</p>
                    <p style={{ fontSize:'13px', fontWeight:500, color:'#1C2B32', margin:0 }}>{item.value}</p>
                  </div>
                ))}
              </div>
              <div>
                <p style={{ fontSize:'10px', color:'#7A9098', letterSpacing:'0.5px', marginBottom:'8px', textTransform:'uppercase' as const }}>Properties ({selectedProperties.length})</p>
